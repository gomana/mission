    ...
    function display_html_content($content = '', $html = '', $thumb_width=700, $autolink = false, $popup = false, $writer_is_admin = false)
    {
        if (empty($html)) {
            ...
            $content = preg_replace_callback(
                ...
                function ($matches) {
                    global $thumb_width;
                    return get_google_map($matches[0], $thumb_width);
                },
                $content
            ); // Google Map
...
if ( ! function_exists('check_cache_dir')) {
	...
	//page 작업
	function getPaging($link_str, $start, $scale, $page, $page_scale, $total, $page_param)
	{
		$html = "";
		...
		if( $link_str=="" ) $link_str = "&".$page_param;
		//페이지 나누는 기능
		//if($total > $page_scale) {
			...
			if($start + 1 > $scale * $page_scale) {
				...
				$html .= "<li><a href='$link_str?start=$pre_start$page_param' >«</a></li>";
			}else{
				$html .= "<li><a >«</a></li>";
			}

			for($cnt=0 ; $cnt < $scale ; $cnt++) {
				$ln = ($page * $scale + $cnt) * $page_scale;
				$vk = $page * $scale + $cnt + 1;

				if($ln < $total) {

					if($ln != $start) {
						$html .= "<li><a href='$link_str?start=$ln$page_param'>$vk</a></li>";
					} else {
						$html .= "<li class='active'><a >$vk</a></li>";
					}
				}
			}
			...
			if($total > (($page + 1) * $scale * $page_scale)) {
				...
				$html .= "<li><a href='$link_str?start=$n_start$page_param' >»</a></li>";
			}else{
				$html .= "<li><a >»</a></li>";
			}
	...
	/**
	 * @param unknown $totalRecord 전체레코드
	 * @param unknown $recordPerPage 페이지당 레코드수(10)
	 * @param unknown $pagePerBlock 블럭당페이지수(10)
	 * @param unknown $currentPage 현재페이지
	 * @param unknown $search_param 검색용 추가 파라미터. 배열형식으로
	 */
	function handlePaging($totalRecord, $recordPerPage, $pagePerBlock, $currentPage, $search_param){

	    //파라미터 정리
	    unset($search_param['page']);
	    $addParam = http_build_query($search_param);
	    if(!empty($addParam)) $addParam ="&".$addParam;
	    ...
	    //echo("startPage:".$startPage."endPage:".$endPage );
	    ...
	    //이전, 이후 페이지
	    $prevPageNo = ($currentPage - 1) > 0 ? ($currentPage - 1) : 1 ;
	    $nextPageNo = ($currentPage + 1) <= $totalNumOfPage ? ($currentPage +  1) : $totalNumOfPage ;
	    ...
	    if(1 == $totalNumOfPage){
	        ...
	        $isPrevPage = false;
	        $isNextPage = true;
	    }else if ( $currentPage == $totalNumOfPage){
	        $isPrevPage = true;
	        $isNextPage = false;
	    }
	    ...
	    //if($isPrev){
	    $goPrevPage = 1; // 1page
	    //##echo "<a href=\"/".filter_var(uri_string(), FILTER_SANITIZE_STRING)."?page=$goPrevPage$addParam\" class='btn-first'>이전 블록</a></li>"; //이전 블록

	    //}
	    //else
	    //{
	    //    echo "<a href='#none' class='btn-first'>처음</a>"; //첫블록
	    /// }

	    if($isPrev){
	        $goPrevPage = $startPage-$pagePerBlock; // 11page
	        echo "<a href='#' onclick='load_entry_list(\"page=$goPrevPage$addParam\");return false;' class='btn_prev'>이전</a></li>"; //이전 블록
	    }
	    else
	    {
	        echo "<a href='#none' class='btn_prev'>이전</a>"; //이전 블록
	    }
	    /*
	     if($isPrev){
	     $goPrevPage = $startPage-$pagePerBlock; // 11page
	     ...
	     {
	     echo "<a href='#none' class='btn-first'>이전 블록</a>"; //이전 블록
	     }

	     echo "<a href='".($isPrevPage ? "/".filter_var(uri_string(), FILTER_SANITIZE_STRING)."?page=$prevPageNo$addParam" : "#none")."' class='btn-prev'>이전</a>";
	   ...
	   // echo "<span class='num'>";
	    for($i=$startPage;$i<=$endPage;$i++){
	        ...
	        {
	            //echo "<a href=\"/".filter_var(uri_string(), FILTER_SANITIZE_STRING)."?page=$i$addParam\"  class='active' >".$i."</a>";
	            echo "<a href='#' onclick='load_entry_list(\"page=$i$addParam\");return false;' class='active' >".$i."</a>";

	        }
	        else
	        {
	            //echo "<a href=\"/".filter_var(uri_string(), FILTER_SANITIZE_STRING)."?page=$i$addParam\">".$i."</a>";
	            echo "<a href='#' onclick='load_entry_list(\"page=$i$addParam\");return false;' >".$i."</a>";
	        }
	    }
	    //echo "</span>";
	    ...
	    if($isNext){
	        ...
	        echo "<a href='#' onclick='load_entry_list(\"page=$goNextPage$addParam\");return false;' class='btn_next'  >다음</a>";   //다음 블록
	    }
	    else
	    {
	        echo "<a href='#none' class='btn_next' >다음</a>";  //다음 블록
	    }

	    //if($isNext){
	    $goNextPage = $totalNumOfPage;
	    //###echo "<a href=\"/".filter_var(uri_string(), FILTER_SANITIZE_STRING)."?page=$goNextPage$addParam\" class='btn-last'  >끝</a></li>";   //다음 블록

	    // }
	    //else
	    //{
	    //    echo "<a href='#none' class='btn-last' >끝</a>";  //다음 블록
	    // }

	    /*
	     echo "<a href='".($isNextPage ? "/".filter_var(uri_string(), FILTER_SANITIZE_STRING)."?page=$nextPageNo$addParam" : "#")."' class='btn-next' >다음</a>";
	...
	function GetStr($str,$strName,$strValue){
		...
		$return_str = $str . $strName ."=". $strValue;
		return $return_str;
	}
...
/*
	//암호화 인코딩
if ( ! function_exists('fEncode')) {
	function fEncode($str){
		$CI = & get_instance();
		$CI->load->library('encrypt');
		$strEncode = new CI_Encrypt();
		return $strEncode->encode($str);
	}
}

	//암호화 디코딩
if ( ! function_exists('fDecode')) {
	function fDecode($str){
		$CI = & get_instance();
		$CI->load->library('encrypt');
		$strDecode = new CI_Encrypt();
		return $strDecode->decode($str);
	}
}
*/
...
if ( ! function_exists('fEncode')) {
	function fEncode($str){
		...
		$strEncode = new CI_Encryption();
		return $strEncode->encrypt($str);
	}
}
...
if ( ! function_exists('fDecode')) {
	function fDecode($str){
		$CI = & get_instance();
		$CI->load->library('encryption');
		$strDecode = new CI_Encryption();
		return $strDecode->decrypt($str);
	}
}


	//관리자계정
if ( ! function_exists('fGetAdminId')) {
	function fGetAdminId(){
		$CI = & get_instance();
		$CI->load->library('session');
		return fDecode($CI->session->userdata('adminId'));
	}
}

	//관리자이름
if ( ! function_exists('fGetAdminName')) {
	function fGetAdminName(){
		$CI = & get_instance();
		$CI->load->library('session');
		return fDecode($CI->session->userdata('adminName'));
	}
}


	//관리자 메뉴세션 초기화
if ( ! function_exists('fGetAdminMenuSessionInit')) {
	function fGetAdminMenuSessionInit(){
		$CI = & get_instance();
		$CI->load->library('session');

		$CI->session->set_userdata('H_MENU1_CODE', "");		//top 선택메뉴 세션값
		$CI->session->set_userdata('H_MENU2_CODE', "");		//left 선택메뉴 세션값
		$CI->session->set_userdata('H_MENU2_READ_YN', "");	//화면 읽기권한 세션값
		$CI->session->set_userdata('H_MENU2_WRITE_YN', "");	//화면 쓰기권한 세션값
...
//엑셀 셀 처리
if(! function_exists('fDigit2ExcelCol')){

    function fDigit2ExcelCol($argNUM=1)
    {
...
if(! function_exists('rtnShowText')){
...
//=====================
//도메인가져오기 ex)https://aaa.bbb.com
if(! function_exists('fGetSiteDomain')){
	function fGetSiteDomain(){
		$protocol="http";
